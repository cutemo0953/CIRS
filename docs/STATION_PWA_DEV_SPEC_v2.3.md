# xIRS Station & Pharmacy PWA Development Specification v2.3

**Version**: 2.3 (Final Merged)
**Date**: 2025-12-22
**Status**: Ready for Development
**Supersedes**: v2.0, v2.2
**Contributors**: Claude, ChatGPT, Gemini

---

## 0. 變更摘要 (Change Summary)

| Source | Contribution |
|--------|--------------|
| **v2.0** | Onboarding UX, Task-oriented dashboard |
| **v2.2** | Data sensitivity model, Dual station types, SOAP-Lite |
| **Claude** | Web Crypto over SQLCipher, CERT_UPDATE packet |
| **ChatGPT** | Pairing security (no embedded secrets), patient_ref hardening |
| **Gemini** | Final architecture consolidation |

---

## 1. 系統架構 (Architecture)

### 1.1 雙 PWA 策略 (Dual PWA Strategy)

**決策理由**:
- 避免「模式切換」造成現場誤用
- 減少資安外溢範圍 (Blast Radius)
- 符合 JTBD「任務一目瞭然」原則

| PWA | URL | 使用者 | 核心任務 | 資料權限 |
|-----|-----|--------|----------|----------|
| **Supply Station** | `/station/` | 物資志工、站長 | 收貨、發放、盤點 | Class C only |
| **Pharmacy Station** | `/pharmacy/` | 藥師 | 接收處方、驗證、發藥 | Class B + C |

### 1.2 資料敏感度分級 (Data Sensitivity Model)

| 分級 | 定義 | 內容範例 | 規則 |
|------|------|----------|------|
| **Class A** (PHI) | 可直接識別個人 | 姓名、身分證、電話、完整病歷 | 🚫 **禁止**存在任何 Station/Pharmacy 本地端 |
| **Class B** (Clinical) | 最小化臨床資料 | `patient_ref` token、SOAP-Lite | 🔒 Pharmacy 專用，AES-GCM 加密儲存 |
| **Class C** (Ops) | 運作資料 | 物品代碼、數量、Station ID、時間戳 | ✅ 全系統通行 |

> **關鍵約束**: CIRS/Station 的任何資料表都**不得**儲存 Class A 或 Class B 資料。

### 1.3 專案結構

```
/frontend
├── /station          # Supply Station PWA
│   ├── index.html
│   ├── manifest.json
│   └── service-worker.js
│
├── /pharmacy         # Pharmacy Station PWA (NEW)
│   ├── index.html
│   ├── manifest.json
│   └── service-worker.js
│
└── /shared           # 共用模組
    ├── /js
    │   ├── xirs-crypto.js      # WebCrypto wrappers
    │   ├── xirs-protocol.js    # Manifest/Report
    │   ├── xirs-storage.js     # IndexedDB
    │   └── xirs-pairing.js     # Pairing flow (NEW)
    └── /components             # Shared UI components
```

---

## 2. 安全配對協議 (Secure Pairing Protocol)

### 2.1 設計原則 (ChatGPT Feedback)

**問題**: 原設計將 `station_secret`、`hub_public_key` 直接嵌入 QR Code。
**風險**: QR 洩漏 = 長期金鑰洩漏 = 永久危害。

**解決方案**: QR 僅含短期配對資訊，金鑰透過安全通道取得。

### 2.2 配對 QR 格式 (v2.3)

```json
{
  "type": "STATION_PAIR_INVITE",
  "ver": 1,
  "hub_url": "https://hub.example.com",
  "pairing_code": "A3X9K2",           // 6-char, expires in 10 min
  "station_id": "STATION-PARK",       // Pre-assigned ID
  "station_type": "SUPPLY"            // or "PHARMACY"
}
```

### 2.3 配對流程

```
┌─────────────────────────────────────────────────────────────────┐
│  PAIRING FLOW                                                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  [Hub Admin]                    [Station PWA]                   │
│       │                              │                          │
│       │  1. Create Station           │                          │
│       │  2. Generate Pairing QR      │                          │
│       │     (code expires 10min)     │                          │
│       │                              │                          │
│       │         ─── QR Code ───►     │                          │
│       │                              │                          │
│       │                         3. Scan QR                      │
│       │                         4. Extract hub_url + code       │
│       │                              │                          │
│       │     ◄─── HTTPS POST ────     │                          │
│       │     /api/stations/pair       │                          │
│       │     { code, device_info }    │                          │
│       │                              │                          │
│       │  5. Validate code            │                          │
│       │  6. Generate station_secret  │                          │
│       │  7. Return config bundle     │                          │
│       │                              │                          │
│       │     ─── Config Bundle ───►   │                          │
│       │     {                        │                          │
│       │       station_secret,        │                          │
│       │       hub_public_key,        │                          │
│       │       hub_encryption_key,    │                          │
│       │       prescriber_certs[]     │  (Pharmacy only)         │
│       │     }                        │                          │
│       │                              │                          │
│       │                         8. Store securely               │
│       │                         9. Ready!                       │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### 2.4 離線配對備援 (XSDEP USB)

若無網路連線，Hub 可匯出簽章設定檔：

```json
{
  "type": "STATION_CONFIG_OFFLINE",
  "ver": 1,
  "station_id": "STATION-PARK",
  "station_type": "SUPPLY",
  "config": {
    "station_secret": "base64...",
    "hub_public_key": "base64...",
    "hub_encryption_key": "base64..."
  },
  "issued_at": 1734912000,
  "expires_at": 1735516800,           // 7 days
  "signature": "Hub_Ed25519_Signature"
}
```

Station 驗證簽章後匯入設定。

### 2.5 Hub Admin UI (配對介面)

Hub 管理員可透過 Admin Console 產生 Station/Pharmacy 配對碼：

**入口**: 系統選單 → 「Station 配對」

**UI 功能**:
1. **站點類型選擇**: 物資站 (SUPPLY) 或 藥局站 (PHARMACY)
2. **站點 ID 輸入**: 例如 `STATION-001`
3. **站點名稱輸入**: 例如「一樓物資站」
4. **產生配對碼**: 6 字元英數字碼，有效期 10 分鐘
5. **QR Code 顯示**: 包含 `STATION_PAIR_INVITE` payload
6. **已配對站點列表**: 顯示所有已完成配對的站點

**API 端點**:
- `POST /api/satellite/stations/generate-pairing` - 產生配對碼
- `POST /api/satellite/stations/pair` - 完成配對
- `GET /api/satellite/stations` - 列出已配對站點

---

## 3. 使用者流程 (User Flows)

### 3.1 首次使用引導 (Onboarding)

```
┌─────────────────────────────────────────────────────────────────┐
│  Step 1: Welcome                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│         [xIRS Station Icon]                                     │
│                                                                  │
│         歡迎使用 xIRS 物資站                                     │
│         (或「藥局工作站」for /pharmacy/)                        │
│                                                                  │
│    這是一個離線優先的物資管理工具，                               │
│    用於災難時期的分站物資發放。                                   │
│                                                                  │
│         [開始設定]                                               │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│  Step 2: Pair with Hub                                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│    請掃描 Hub 提供的配對 QR Code                                 │
│                                                                  │
│         ┌─────────────────┐                                     │
│         │   [Camera]      │                                     │
│         └─────────────────┘                                     │
│                                                                  │
│    ─────── 或 ───────                                           │
│                                                                  │
│    Hub URL:  [________________]                                 │
│    配對碼:   [______]                                           │
│                                                                  │
│    ─────── 完全離線？ ───────                                   │
│                                                                  │
│    [匯入離線設定檔 (.json)]                                      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│  Step 3: Ready                                                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│         ✓ 配對成功！                                            │
│                                                                  │
│    Station: 公園站 (STATION-PARK)                               │
│    Hub: 社區活動中心                                             │
│    類型: 物資站                                                  │
│                                                                  │
│         [開始使用]                                               │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### 3.2 Supply Station 儀表板 (`/station/`)

```
┌─────────────────────────────────────────────────────────────────┐
│  [≡]              公園站 - 物資站            [🔔 2] [⚙️]        │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌─────────────────────┐  ┌─────────────────────┐              │
│  │                     │  │                     │              │
│  │   📥 收貨           │  │   📤 發放           │              │
│  │   Receive           │  │   Dispense          │              │
│  │                     │  │                     │              │
│  │   掃描 Runner       │  │   發放物資給        │              │
│  │   帶來的補給清單    │  │   避難者            │              │
│  │                     │  │                     │              │
│  └─────────────────────┘  └─────────────────────┘              │
│                                                                  │
│  ┌─────────────────────┐  ┌─────────────────────┐              │
│  │                     │  │                     │              │
│  │   📋 回報           │  │   👤 登記           │              │
│  │   Report            │  │   Register          │              │
│  │                     │  │                     │              │
│  │   產生報告給        │  │   登記新抵達        │              │
│  │   Runner 帶回       │  │   的避難者          │              │
│  │                     │  │                     │              │
│  └─────────────────────┘  └─────────────────────┘              │
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
│  ┌──────────┐ ┌──────────┐ ┌──────────┐                        │
│  │    12    │ │    45    │ │    3     │                        │
│  │  物資品項 │ │  登記人數 │ │  待回報  │                        │
│  └──────────┘ └──────────┘ └──────────┘                        │
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
│  [🏠 首頁]    [📦 庫存]    [📋 歷史]    [⚙️ 設定]              │
└─────────────────────────────────────────────────────────────────┘
```

### 3.3 Pharmacy Station 儀表板 (`/pharmacy/`)

```
┌─────────────────────────────────────────────────────────────────┐
│  [≡]              公園藥局 - 藥局站          [🔔 5] [⚙️]        │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌─────────────────────┐  ┌─────────────────────┐              │
│  │                     │  │                     │              │
│  │   📷 掃描處方       │  │   💊 發藥佇列       │              │
│  │   Scan Rx           │  │   Dispense Queue    │              │
│  │                     │  │                     │              │
│  │   掃描醫師開立的    │  │   待發藥: 8 筆      │              │
│  │   處方 QR Code      │  │   管制藥: 2 筆      │              │
│  │                     │  │                     │              │
│  └─────────────────────┘  └─────────────────────┘              │
│                                                                  │
│  ┌─────────────────────┐  ┌─────────────────────┐              │
│  │                     │  │                     │              │
│  │   📥 進藥           │  │   📋 回報           │              │
│  │   Stock             │  │   Report            │              │
│  │                     │  │                     │              │
│  │   掃描藥品補給      │  │   同步發藥紀錄      │              │
│  │   清單              │  │   至 Hub            │              │
│  │                     │  │                     │              │
│  └─────────────────────┘  └─────────────────────┘              │
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
│  ┌──────────┐ ┌──────────┐ ┌──────────┐                        │
│  │    28    │ │    8     │ │    12    │                        │
│  │  藥品品項 │ │  待發藥  │ │  待回報  │                        │
│  └──────────┘ └──────────┘ └──────────┘                        │
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
│  [🏠 首頁]    [💊 藥品]    [📋 稽核]    [⚙️ 設定]              │
└─────────────────────────────────────────────────────────────────┘
```

---

## 4. 核心功能規格 (Core Specifications)

### 4.1 物流循環 (Logistics Loop) - 共用

**收貨 (Receive)**:
1. 掃描 `RESTOCK_MANIFEST` QR
2. 驗證 Ed25519 簽章
3. 檢查 `manifest_id` 去重
4. 顯示物資清單供確認
5. 確認後更新 `local_inventory`
6. 記錄 `RECEIVE` action

**發放 (Dispense)**:
1. 選擇物資 + 數量
2. 可選：掃描/輸入領取人 ID
3. 扣減 `local_inventory`
4. 記錄 `DISPENSE` action

### 4.1.1 處置領用單 (Consumption Ticket) - v2.3.1 新增

> 整合 `xIRS_PROCEDURE_INVENTORY_BRIDGE_SPEC_v1.0.md`
> 支援臨床處置 → 物資扣帳的鬆耦合連結

**掃描類型擴充**:

| 類型 | 說明 | 驗證方式 |
|------|------|----------|
| `RESTOCK_MANIFEST` | 進貨清單 | Ed25519 (Hub) |
| `CONSUMPTION_TICKET` | 處置領用單 | HMAC (station_secret) |
| `CONSUMPTION_RETURN` | 退還單 | HMAC (station_secret) |

**CONSUMPTION_TICKET 處理流程**:

1. 掃描 `CONSUMPTION_TICKET` QR
2. 驗證 HMAC (使用 station_secret)
3. 檢查 `ticket_id` 去重
4. 顯示領用清單 (無病患資訊):
   ```
   ┌─────────────────────────────────────┐
   │  領用單驗證                         │
   │  Event: evt_a1b2***                 │
   │  來源: NURSE-STATION-A              │
   │                                     │
   │  品項:                              │
   │  ☑ DEV-ETTUBE-7.5 x 1              │
   │  ☑ DEV-ETTUBE-7.0 x 1              │
   │  ☑ DEV-LARYNGO-MAC3 x 1            │
   │                                     │
   │  [確認扣庫]  [取消]                 │
   └─────────────────────────────────────┘
   ```
5. 確認後扣減 `local_inventory`
6. 產生 `CONSUMPTION_RECORD`:
   ```json
   {
     "type": "CONSUMPTION_RECORD",
     "event_ref": "evt_a1b2c3d4",
     "items_used": [...],
     "variance": { "added": [], "removed": [] }
   }
   ```
7. 加入回報佇列

**隱私保護**:
- ❌ 不顯示 `patient_ref`
- ❌ 不顯示 `procedure_code`
- ✅ 只顯示 `event_ref` (遮蔽) 和品項列表
- ✅ 透過 `event_ref` 在 Hub 對帳

**回報 (Report)**:
1. 彙整待同步 `actions`
2. 打包 `REPORT_PACKET`
3. SealedBox 加密 + HMAC
4. 分段產生 QR Code
5. Runner 掃描完成後標記 "in transit"

### 4.2 臨床循環 (Clinical Loop) - Pharmacy Only

**接收處方 (Rx Ingest)**:
1. 掃描 Doctor PWA 的 Rx QR
2. 驗證醫師簽章 (使用本地 `PRESCRIBER_CERT`)
3. 檢查 `rx_id` 去重 (Replay Protection)
4. 展示處方內容:
   - 藥品清單
   - `patient_ref` (顯示末 4 碼)
   - 醫師 ID + 名稱
5. 加入發藥佇列

**發藥 (Dispense Rx)**:
1. 從佇列選擇處方
2. 確認/調整藥品 (替代用藥)
3. 管制藥需填寫見證人
4. 提交:
   - 扣減藥品庫存
   - 產生 `DISPENSE_RECORD` (Class B, 加密)
   - 記錄稽核軌跡

### 4.3 證書更新 (Cert Sync)

**封包格式 (`CERT_UPDATE`)**:

```json
{
  "type": "CERT_UPDATE",
  "ver": 1,
  "certs": [
    {
      "id": "DOC-001",
      "name": "王大明 醫師",
      "public_key": "base64_ed25519_pubkey",
      "issued_at": 1734912000,
      "expires_at": 1766448000,
      "revoked": false
    }
  ],
  "issued_at": 1734912000,
  "signature": "Hub_Ed25519_Signature"
}
```

**更新機制**:
1. Hub 產生簽章的 `CERT_UPDATE` QR
2. Runner 帶到 Pharmacy
3. Pharmacy 掃描 → 驗簽 → 更新本地 `prescribers` store
4. 過期/撤銷的憑證自動移除

---

## 5. 技術實作 (Technical Implementation)

### 5.1 IndexedDB Schema

**Supply Station (`/station/`)**:

```javascript
const STATION_STORES = {
  config: 'keyval',           // Station config
  inventory: 'keyval',        // code -> { qty, unit, name }
  actions: 'autoIncrement',   // Action log (pending sync)
  manifests: 'keyval',        // Processed manifest IDs
  reports: 'keyval',          // Generated report history
  persons: 'autoIncrement'    // Registered persons
};
```

**Pharmacy Station (`/pharmacy/`)**:

```javascript
const PHARMACY_STORES = {
  ...STATION_STORES,
  prescribers: 'keyval',      // Prescriber certs
  rx_queue: 'autoIncrement',  // Pending Rx to dispense
  rx_history: 'keyval',       // Processed rx_ids (dedupe)
  clinical: 'autoIncrement'   // Encrypted clinical records
};
```

### 5.2 加密實作 (Web Crypto API)

```javascript
// xirs-crypto.js

class ClinicalEncryption {
  constructor() {
    this.algorithm = { name: 'AES-GCM', length: 256 };
  }

  // Derive key from station_secret
  async deriveKey(stationSecret) {
    const keyMaterial = await crypto.subtle.importKey(
      'raw',
      base64ToArrayBuffer(stationSecret),
      'PBKDF2',
      false,
      ['deriveKey']
    );

    return crypto.subtle.deriveKey(
      {
        name: 'PBKDF2',
        salt: new TextEncoder().encode('xirs-clinical-v1'),
        iterations: 100000,
        hash: 'SHA-256'
      },
      keyMaterial,
      this.algorithm,
      false,
      ['encrypt', 'decrypt']
    );
  }

  // Encrypt Class B data
  async encrypt(data, key) {
    const iv = crypto.getRandomValues(new Uint8Array(12));
    const encoded = new TextEncoder().encode(JSON.stringify(data));

    const ciphertext = await crypto.subtle.encrypt(
      { name: 'AES-GCM', iv },
      key,
      encoded
    );

    return {
      iv: arrayBufferToBase64(iv),
      ciphertext: arrayBufferToBase64(ciphertext)
    };
  }

  // Decrypt Class B data
  async decrypt(encrypted, key) {
    const iv = base64ToArrayBuffer(encrypted.iv);
    const ciphertext = base64ToArrayBuffer(encrypted.ciphertext);

    const decrypted = await crypto.subtle.decrypt(
      { name: 'AES-GCM', iv },
      key,
      ciphertext
    );

    return JSON.parse(new TextDecoder().decode(decrypted));
  }
}
```

### 5.3 處方 QR 格式 (Rx QR)

```json
{
  "type": "RX_ORDER",
  "ver": 1,
  "rx_id": "RX-20251222-001",
  "prescriber_id": "DOC-001",
  "patient_ref": "pref_a1b2c3d4e5f6",   // Token only, no name!
  "items": [
    { "code": "AMOX-500", "qty": 21, "dosage": "1# TID x 7D" },
    { "code": "ACET-500", "qty": 10, "dosage": "1# PRN" }
  ],
  "notes_encrypted": "base64...",        // Optional SOAP-Lite (encrypted)
  "issued_at": 1734912000,
  "expires_at": 1735516800,              // 7 days
  "signature": "Prescriber_Ed25519_Sig"
}
```

> **隱私要點**: `patient_ref` 是 Hub 產生的不可逆 token，Pharmacy 無法從中推導出病患身份。

---

## 6. UI/UX 規格 (Design Specifications)

### 6.1 設計原則

| 原則 | 實作 |
|------|------|
| **任務導向** | 大按鈕 + 清晰說明文字 |
| **離線可見** | 永遠顯示同步狀態、離線 banner |
| **防呆設計** | 破壞性操作需確認 |
| **扁平設計** | Teal + Deep Blue，無漸層 |
| **Heroicons** | 禁止使用 emoji |

### 6.2 色彩系統

```css
/* Supply Station - Teal */
--station-600: #0d9488;
--station-700: #0f766e;
--station-800: #115e59;

/* Pharmacy Station - Indigo */
--pharmacy-600: #4f46e5;
--pharmacy-700: #4338ca;
--pharmacy-800: #3730a3;

/* Shared */
--accent-600: #1e3a5f;    /* Deep blue */
--success: #10b981;
--warning: #f59e0b;
--error: #ef4444;
```

### 6.3 隱私模式 Toggle

```
┌─────────────────────────────────────────┐
│  設定                                    │
├─────────────────────────────────────────┤
│                                          │
│  🔒 隱私模式           [████████ ON]    │
│                                          │
│  開啟時：                                │
│  • patient_ref 僅顯示末 4 碼             │
│  • 隱藏所有人口學資料                    │
│  • 發藥紀錄不顯示註記                    │
│                                          │
└─────────────────────────────────────────┘
```

---

## 7. API 規格 (Backend API)

### 7.1 配對 API

```
POST /api/stations/pair
Content-Type: application/json

Request:
{
  "pairing_code": "A3X9K2",
  "device_info": {
    "user_agent": "...",
    "platform": "iOS"
  }
}

Response (200):
{
  "station_id": "STATION-PARK",
  "station_type": "SUPPLY",
  "display_name": "公園站",
  "config": {
    "station_secret": "base64...",
    "hub_public_key": "base64...",
    "hub_encryption_key": "base64..."
  },
  "prescriber_certs": []  // Only for PHARMACY type
}

Response (401):
{
  "error": "INVALID_OR_EXPIRED_CODE"
}
```

### 7.2 報告接收 API

```
POST /api/logistics/reports
Content-Type: application/json

Request:
{
  "station_id": "STATION-PARK",
  "encrypted_payload": "base64_sealedbox...",
  "hmac": "base64..."
}

Response (200):
{
  "packet_id": "PKT-a1b2c3d4",
  "status": "ACCEPTED",
  "actions_processed": 12
}
```

---

## 8. 驗收標準 (Acceptance Criteria)

### 8.1 功能驗收

| # | 測試項目 | 預期結果 |
|---|----------|----------|
| 1 | `/station/` 頁面無處方相關功能 | 找不到「掃描處方」按鈕或代碼 |
| 2 | `/pharmacy/` 頁面無人員登記功能 | 找不到「人員登記」按鈕 |
| 3 | 離線驗證 Rx | 飛航模式下能驗證有效 Rx 並拒絕無效簽章 |
| 4 | 盲傳安全 | Runner QR 內容無 Hub 私鑰無法解讀 |
| 5 | 配對流程 | 新裝置 1 分鐘內完成配對 |

### 8.2 隱私驗收

| # | 測試項目 | 預期結果 |
|---|----------|----------|
| 1 | Station IndexedDB 檢查 | 無 Class A/B 資料 |
| 2 | Pharmacy IndexedDB 檢查 | Class B 資料皆已加密 |
| 3 | 處方 QR 檢查 | 無病患姓名，僅有 `patient_ref` |
| 4 | Report Packet 檢查 | 無明文臨床資料 |

---

## 9. 實作優先序 (Implementation Priority)

### Phase 1: Supply Station Redesign
- [ ] Onboarding flow (Pairing)
- [ ] Task-oriented dashboard
- [ ] Improved receive/dispense UX
- [ ] Visual polish (Teal + Heroicons)

### Phase 2: Pharmacy Station (New)
- [ ] Create `/pharmacy/` PWA
- [ ] Rx scanning + verification
- [ ] Dispense queue
- [ ] Clinical encryption

### Phase 3: Hub Integration
- [ ] Pairing API (`/api/stations/pair`)
- [ ] CERT_UPDATE generation
- [ ] Clinical report ingestion

### Phase 4: Polish
- [ ] Privacy mode toggle
- [ ] Offline config import
- [ ] Error handling & edge cases

---

## 10. 附錄 (Appendix)

### A. 術語對照

| Term | 中文 | 說明 |
|------|------|------|
| Station | 分站 | 離線運作的物資發放點 |
| Pharmacy | 藥局站 | 具備處方驗證能力的分站 |
| Hub | 指揮中心 | 具權威性的中央伺服器 |
| Runner | 傳令兵 | 盲傳 QR Code 的人員 |
| Manifest | 出貨清單 | Hub 簽章的物資清單 |
| Report | 回報封包 | Station 加密的動作紀錄 |
| Rx | 處方 | 醫師簽章的藥品處方 |
| patient_ref | 病患代號 | 不可逆的病患識別 token |

### B. 相關文件

- [xIRS_DISTRIBUTED_LOGISTICS_SPEC_v1.8.md](./xIRS_DISTRIBUTED_LOGISTICS_SPEC_v1.8.md)
- [xIRS_LITE_CPOE_SPEC_v1.0.md](./xIRS_LITE_CPOE_SPEC_v1.0.md)
- [xIRS_ARCHITECTURE_ADJUSTMENTS_v2.md](./xIRS_ARCHITECTURE_ADJUSTMENTS_v2.md)

---

**Document Version**: 2.3
**Created**: 2025-12-22
**Contributors**: Claude, ChatGPT, Gemini
**Status**: Ready for Development
