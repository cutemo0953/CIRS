...
### 5.2 Phase 2 - 簽章驗證 (規劃中)

```
signature = HMAC-SHA256(
  key = site_secret,
  message = JSON.stringify(payload, sorted_keys)
)
```

**流程：**
1. CIRS 產生 QR 時，用站點密鑰簽章
2. HIRS 掃描時，檢查簽章
3. 簽章無效 → 顯示「不可信來源」警告
4. 使用者可選擇仍要匯入
...
1. **簽章密鑰管理**：站點密鑰如何產生、儲存、輪換？
2. **離線驗證**：HIRS 離線時如何驗證 CIRS 簽章？
3. **範本審核**：社群範本是否需要審核機制？
4. **MIRS 整合**：Triage Tag 連結的具體格式？
5. **多語系**：item_code 是否需要多語系名稱對照？

---

*Document generated for IRS Ecosystem*
*HIRS / CIRS / MIRS Interoperability*
