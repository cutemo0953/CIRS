...
```
v1.4.2 → v1.4.5 (失敗) → v1.4.2-plus → v2.0 → v2.3
   │         │               │            │       │
   │         │               │            │       └── MVP聚焦：移除網路功能
   │         │               │            └── 靜態配置：修復 Station ID 同步問題
   │         │               └── 回退基底 + 新功能（血袋標籤等）
   │         └── Setup Wizard 在 RPi 無法執行 (init 問題)
   └── 原始穩定版本（無 Setup Wizard）
```
...
| 版本 | 狀態 | 說明 |
|------|------|------|
| **v1.4.2** | 穩定 | 原始單站版本，無 Setup Wizard，可在 RPi 上正常執行 |
| **v1.4.5** | ❌ 失敗 | 新增 Setup Wizard + Database Profiles，但在 Raspberry Pi 上無法執行，懷疑 init 問題 |
| **v1.4.2-plus** | ✅ 穩定 | **回退策略**：以 v1.4.2 為基底，將 v1.4.5 的新功能（血袋標籤等）手動移植過來，避開 Setup Wizard 問題 |
| **v2.0** | ✅ 穩定 | 正式採用靜態配置架構，徹底解決 Station ID 同步問題 |
| **v2.3** | ✅ 穩定 | MVP 聚焦版，移除網路傳輸功能 |

### 1.2 關鍵決策：回退 vs 修復

當 v1.4.5 在 RPi 上無法執行時，有兩個選擇：
1. **修復 v1.4.5** - 花時間 debug Setup Wizard 的 init 問題
2. **回退 + 移植** - 用穩定的 v1.4.2 為基底，只移植需要的新功能

最終選擇 **回退 + 移植** 策略，產生 v1.4.2-plus：
- 保留 v1.4.2 的穩定啟動流程
- 手動加入 v1.4.5 的血袋標籤列印等功能
- 不引入 Setup Wizard 的複雜度

**教訓**: 在嵌入式環境（如 RPi），首次啟動流程的複雜度會帶來難以預期的問題。

---

## 2. 單站架構時期 (v1.4.2 ~ v1.4.2-plus)
...
## 3. Setup Wizard 失敗時期 (v1.4.5 / v1.5)

> **注意**: v1.4.5 在 Raspberry Pi 上無法正常執行，懷疑是 init 初始化問題。
> 此版本的功能設計被記錄下來，但實際部署時採用 v1.4.2-plus 繞過此問題。

### 3.1 設計目標（未成功部署）
支援不同類型站點快速部署，首次啟動時選擇站點類型

### 3.2 Database Profiles 系統（設計文件）
```
database/profiles/
...
### 3.3 v1.4.5 在 RPi 上的問題

**問題一：Init 啟動失敗**
- Setup Wizard 在 Raspberry Pi 上無法正常初始化
- 具體原因未完全釐清，懷疑與資料庫初始化流程有關
- 最終決定回退到 v1.4.2 基底

**問題二：Station ID 不同步（即使能啟動）**

假設 Setup Wizard 能正常啟動，仍有以下設計缺陷：
...
### 3.4 決策：放棄 v1.4.5，回退 + 移植

由於 v1.4.5 問題難以快速修復，採取以下策略：
1. 回退到穩定的 v1.4.2
2. 將 v1.4.5 的有用功能（血袋標籤等）手動移植
3. 產出 v1.4.2-plus 作為實際部署版本
4. v2.0 時正式採用靜態配置架構，從根本解決問題
